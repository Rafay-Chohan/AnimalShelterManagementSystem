    ...
    if(!empty($user_name) && !empty($second_name) && !empty($gender)&& !empty($DOB)&& !empty($CNIC)&& !empty($Phone) && !is_numeric($user_name))
      {
        ...
        try
        {
          $query2 = "INSERT INTO RescuePublicData (Rescue_ID,Public_ID,First_Name,Second_Name,Gender,DOB,Phone,Type_Public,DateofProcess) VALUES('$Rescue_ID','$CNIC','$user_name','$second_name','$gender','$DOB','$Phone','Sponsor',CURDATE())";
          $result = mysqli_query($con, $query2);
          if ($result) {
           $query = "insert into Public (First_Name,Second_Name,Gender,DOB,CNIC,Phone,Type_Public) VALUES('$user_name','$second_name','$gender','$DOB','$CNIC','$Phone','Sponsor')";
        ...
        $query3 = "Update Rescues set Status='Sponsored' where Rescue_Id='$Rescue_ID'";
...
<html lang="en">
  <head>
    ...
    <title>Todds Welfare Society - Sponsor</title>
  </head>
  <body>

    <!-- NAV Bar-->
    <nav class="navbar navbar-expand-lg navbar-dark bg-danger">
      <a class="navbar-brand" href="test.php">TWS</a>
        ...
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu active" aria-labelledby="navbarDropdown">
              ...
              <a class="dropdown-item active" href="#">Sponsor <span class="sr-only">(current)</span></a>
            </div>
          </li>
        </ul>
...
<div class="container">
      <div class="row">
        ...
        <div class="col-md-9 mt-3">
              ...
              <table class="table table-bordered table-responsive">
                ...
                <?php
                  ...
                  if(($_type!="ALL") && (isset($_POST['genderbt'])))//if Type And Gender Both are Selected
                  {
                      ...
                      $sql = "SELECT * FROM Rescues WHERE Animal ='$_type' AND Gender ='$_gender' AND Status='Sponsor' " ;
                  }
                  else if($_type!="ALL")//If only Type is selected
                  { 
                    $sql = "SELECT * FROM Rescues WHERE Animal ='$_type' AND Status='Sponsor' "; 
                  }
                  else if(isset($_POST['genderbt']))//If only Gender is Selected
                  {
                    $_gender=$_POST['genderbt'];
                    $sql = "SELECT * FROM Rescues WHERE Gender ='$_gender' AND Status='Sponsor' " ;
                  }
                  else//If none is selected
                  {
                    $sql = "SELECT * FROM Rescues Where Status='Sponsor'" ;
                  }
              ...
              <?php
                  if(!isset($_POST['Register']))
                  {
                    ?>
                      <form method="post">
                      <input type="submit" name="Register" id="Register" value="Sponsor" /><br/>
                      </form>
                    <?php
        ...
        <div class="screen">
                  <div class="signUp">
                      <h3 class="text-center">Sponsor Rescue</h3>
                          ...
                          <form class="needs-validation" method="post" action="#">
                                  ...
                                  <?php
                                      $sql = "SELECT Rescue_ID FROM Rescues Where Status ='Sponsor' ";
                              ...
                              <div class="form-group">
                                  <input type="submit" value="Sponsor" class="btn btn-outline-danger w 100">
                              </div>
                          </form>
                  </div>
            </div>
